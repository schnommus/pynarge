uniform float sigma=3;     // The sigma value for the gaussian function: higher value means more blur
...
void main() {
  ...
  avgValue += texture2D(texture, gl_TexCoord[0].xy) * incrementalGaussian.x;
  coefficientSum += incrementalGaussian.x;
  incrementalGaussian.xy *= incrementalGaussian.yz;

  // Go through the remaining 8 vertical samples (4 on each side of the center)
  for (float i = 1.0f; i <= numBlurPixelsPerSide; i++) { 
    avgValue += texture2D(texture, gl_TexCoord[0].xy - i * blurSize * 
